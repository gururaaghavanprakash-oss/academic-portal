...
def load_data(filename):
    # This new 'try-except' block stops the app from crashing if a file gets corrupted
    try:
        ...
        # If anything goes wrong, return a clean slate instead of crashing
        if filename in ['questions.json', 'deletion_requests.json']:
            return []
        return {}
...
if not st.session_state.logged_in:
    ...
    if portal == "Professor":
        ...
        if st.button("Professor Login"):
            if prof_id and passcode:
                ...
                if prof_id in creds and isinstance(creds[prof_id], dict):
                    if creds[prof_id].get("passcode") == passcode:
                        st.session_state.logged_in = True
                        st.session_state.role = "Professor"
                        st.session_state.username = prof_id
                        st.session_state.institution = creds[prof_id].get("institution", "")
        ...
        insts = load_data('institutions.json')
        approved_insts = [k for k, v in insts.items() if isinstance(v, dict) and v.get("status") == "approved"]
        ...
        if st.button("Admin Login"):
            if admin_id == "ADMIN" and admin_pass == "ATP2026":
                ...
                insts = load_data('institutions.json')
                if admin_id in insts and isinstance(insts[admin_id], dict):
                    if insts[admin_id].get("password") == admin_pass:
                        if insts[admin_id].get("status") in ["approved", "scheduled_for_deletion"]:
                            st.session_state.logged_in = True
                ...
                for k, v in approved.items():
                    ...
                    with col1:
                        if v.get("status") == "scheduled_for_deletion":
                            st.write(f"⚠️ **{v.get('institute_name', 'Unknown')}** - *Pending Auto-Delete on {v.get('deletion_date')}*")
                        else:
                            st.write(f"**{v.get('institute_name', 'Unknown')}** (Admin ID: {k})")
                    with col2:
    ...
    # === 2. INSTITUTION ADMIN DASHBOARD ===
        ...
        if my_inst.get("status") == "scheduled_for_deletion":
            ...
            if st.button("Cancel Deletion Request & Recover Account", type="primary"):
                ...
                if "deletion_date" in insts[st.session_state.institution]:
    ...
    # === 3. PROFESSOR DASHBOARD ===
        ...
        if st.session_state.prof_dept is None:
            ...
            with tab2:
                ...
                my_questions = [q for q in questions if isinstance(q, dict) and q.get("professor") == st.session_state.username and q.get("institution") == st.
